...
{
    private readonly List<UnscaledGlyphPlan> _list = new List<UnscaledGlyphPlan>();
...
public struct GlyphPlanSequence
{
    ...
    //
    private readonly IUnscaledGlyphPlanList _glyphPlanList;
...
{
    private readonly Dictionary<int, GlyphLayoutPlanContext> _collection = new Dictionary<int, GlyphLayoutPlanContext>();
...
{
    private readonly GlyphLayoutPlanCollection _layoutPlanCollection = new GlyphLayoutPlanCollection();
    ...
    private readonly GlyphIndexList _inputGlyphs = new GlyphIndexList();//reusable input glyph
    private readonly GlyphPosStream _glyphPositions = new GlyphPosStream();

    private static readonly ScriptLang s_latin = new ScriptLang("latn");
    private static readonly ScriptLang s_math = new ScriptLang("math");
    ...
    private GlyphNotFoundHandler _glyphNotFoundHandler;

    //not thread-safe***
    private List<int> _reusableUserCodePoints = new List<int>();

#if DEBUG
    private List<dbugCodePointFromUserChar> _dbugReusableCodePointFromUserCharList = new List<dbugCodePointFromUserChar>();
#endif
