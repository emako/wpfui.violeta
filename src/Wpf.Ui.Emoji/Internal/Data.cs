...
public static class EmojiData
{
    ...
    public class Emoji
    {
        ...
        public string DisplayName
        {
            get
            {
                ...
                // Loop through every string inside the array
        ...
        public IList<Emoji> VariationList { get; } = [];
    ...
    public class Group
    {
        ...
        public IList<SubGroup> SubGroups { get; } = [];

        public int EmojiCount
            => SubGroups.Sum(s => s.EmojiList.Count);
    ...
    #endregion Custom emoji sequences
    ...
    private static readonly Dictionary<string, Drawing> m_custom_drawings = [];

    #endregion Custom drawings

    private static string m_match_one_string;

    // FIXME: this could be read directly from emoji-test.txt.gz
    private static readonly List<string> SkinToneComponents =
    [
        "🏻", // light skin tone
    ...
    private static readonly List<string> HairStyleComponents =
    [
        "🦰", // red hair
        "🦱", // curly hair
        "🦳", // white hair
    ...
    {
        Regex match_group = new(@"^# group: (.*)");
        Regex match_subgroup = new(@"^# subgroup: (.*)");
        Regex match_sequence = new(@"^([0-9a-fA-F ]+[0-9a-fA-F]).*; *([-a-z]*) *# [^ ]* (E[0-9.]* )?(.*)");
        Regex match_skin_tone = new($"({string.Join("|", SkinToneComponents)})");
        Regex match_hair_style = new($"({string.Join("|", HairStyleComponents)})");

        var adult = "(👨|👩)(🏻|🏼|🏽|🏾|🏿)?";
        var child = "(👦|👧|👶)(🏻|🏼|🏽|🏾|🏿)?";
        Regex match_family = new($"{adult}(\u200d{adult})*(\u200d{child})+");
