...
public class TaskDialogButton : TaskDialogItem
{
    ...
    /// </summary>
    /// <value>
    /// One of the <see cref="Wpf.Ui.Violeta.Win32.ButtonType"/> values that indicates the type of the button. The default value
    /// is <see cref="Wpf.Ui.Violeta.Win32.ButtonType.Custom"/>.
    /// </value>
    ...
    ///   is <see cref="Wpf.Ui.Violeta.Win32.ButtonType.Custom"/>. For other button types, it is ignored.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    [Localizable(true), Category("Appearance"), Description("The text of the note associated with a command link button."), DefaultValue("")]
        ...
        set
        {
            if (base.Id != value)
            {
                if (_type != ButtonType.Custom)
                    throw new InvalidOperationException(TaskDialogResources.NonCustomTaskDialogButtonIdError);
                base.Id = value;
            }
        }
    }
    ...
    internal TaskDialogNativeMethods.TaskDialogCommonButtonFlags ButtonFlag
    {
        get
        {
            return _type switch
            {
                ButtonType.Ok => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.OkButton,
                ButtonType.Yes => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.YesButton,
                ButtonType.No => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.NoButton,
                ButtonType.Cancel => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.CancelButton,
                ButtonType.Retry => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.RetryButton,
                ButtonType.Close => TaskDialogNativeMethods.TaskDialogCommonButtonFlags.CloseButton,
                _ => 0,
            };
        }
    }
        ...
        if (type != ButtonType.Custom && Owner != null)
        {
            foreach (TaskDialogButton button in Owner.Buttons)
            {
                if (button != this && button != itemToExclude && button.ButtonType == type)
                    throw new InvalidOperationException(TaskDialogResources.DuplicateButtonTypeError);
            }
        }
    }
}
