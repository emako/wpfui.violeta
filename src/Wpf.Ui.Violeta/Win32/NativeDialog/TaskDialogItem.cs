...
public abstract partial class TaskDialogItem : Component
{
    ...
    {
        if (Owner == null)
            throw new InvalidOperationException(TaskDialogResources.NoAssociatedTaskDialogError);
            ...
            if (items != null)
            {
                foreach (TaskDialogItem item in items)
                {
                    if (item != this && item != itemToExclude && item.Id == id)
                        throw new InvalidOperationException(TaskDialogResources.DuplicateItemIdError);
                }
            }
        }
    }
