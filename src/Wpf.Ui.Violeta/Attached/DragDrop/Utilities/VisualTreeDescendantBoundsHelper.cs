...
namespace Wpf.Ui.Violeta.Attached.DragDrop.Utilities;
...
internal static class VisualTreeDescendantBoundsHelper
{
    ...
    private static MethodInfo CheckFlagsAndMethodInfo { get; } = typeof(Visual).GetMethod("CheckFlagsAnd", BindingFlags.Instance | BindingFlags.NonPublic, null, [VisualFlagsType], null)!;
    ...
    private static MethodInfo UnitToWorldMethodInfo { get; } = typeof(Effect).GetMethod("UnitToWorld", BindingFlags.Static | BindingFlags.NonPublic, null, [typeof(Rect), typeof(Rect)], null)!;
    ...
    public static Rect GetVisibleDescendantBounds(Visual visual)
    {
        ...
        if ((bool)RectHasNaNMethodInfo.Invoke(null, [boundingBoxSubGraph])!)
        {
    ...
    private static Rect CalculateSubgraphBoundsInnerSpace(Visual visual, bool renderBounds)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (VisualTreeHelper.GetChild(visual, i) is Visual child
                && ((bool)CheckFlagsAndMethodInfo.Invoke(child, [Enum.Parse(VisualFlagsType, "VisibilityCache_Visible")])! || (bool)CheckFlagsAndMethodInfo.Invoke(child, [Enum.Parse(VisualFlagsType, "VisibilityCache_TakesSpace")])!))
            {
        ...
        if (renderBounds && (bool)IsEmptyRenderBoundsMethodInfo.Invoke(visual, [contentBounds])!)
        {
    ...
    private static Rect CalculateSubgraphBoundsOuterSpace(Visual visual, bool renderBounds)
    {
        ...
        if ((bool)CheckFlagsAndMethodInfo.Invoke(visual, [Enum.Parse(VisualFlagsType, "NodeHasEffect")])!)
        {
            Effect effect = (Effect)GetValueMethodInfo(typeof(Effect)).Invoke(EffectFieldFieldInfo.GetValue(visual), [visual])!;

            if (effect != null)
            {
                Rect unitBounds = new(0, 0, 1, 1);
                Rect unitTransformedBounds = (EffectMappingPropertyInfo.GetValue(effect) as GeneralTransform)!.TransformBounds(unitBounds);
                Rect effectBounds = (Rect)UnitToWorldMethodInfo.Invoke(null, [unitTransformedBounds, boundingBoxSubGraph])!;

                boundingBoxSubGraph.Union(effectBounds);
            }
            else
            {
                Debug.Assert(GetValueMethodInfo(BitmapEffectStateType).Invoke(BitmapEffectStateFieldFieldInfo.GetValue(visual), [visual]) != null);
            }
        }

        Geometry clip = (Geometry)GetValueMethodInfo(typeof(Geometry)).Invoke(ClipFieldFieldInfo.GetValue(visual), [visual])!;
        ...
        Transform transform = (Transform)GetValueMethodInfo(typeof(Transform)).Invoke(TransformFieldFieldInfo.GetValue(visual), [visual])!;

        if ((transform != null) && (!(bool)IsIdentityPropertyInfo.GetValue(transform)!))
        {
            Matrix matrix = transform.Value;
            TransformRectMethodInfo.Invoke(null, [boundingBoxSubGraph, matrix]);
        }
        ...
        Rect? scrollClip = (Rect?)GetValueMethodInfo(typeof(Rect?)).Invoke(ScrollableAreaClipFieldFieldInfo.GetValue(visual), [visual]);
