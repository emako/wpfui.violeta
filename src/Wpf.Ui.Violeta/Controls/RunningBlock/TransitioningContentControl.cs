...
{
    private FrameworkElement? _contentPresenter = null;

    private static readonly Lazy<Storyboard> StoryboardBuildInDefault = new(() =>
        ResourceHelper.GetResource<Storyboard>($"{default(TransitionMode)}Transition") ?? throw new NotImplementedException(), isThreadSafe: true);
    ...
    {
        _storyboardBuildIn = ResourceHelper.GetResource<Storyboard>($"{newValue}Transition") ?? throw new NotImplementedException();
        StartTransition();
    }
