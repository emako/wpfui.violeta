...
public sealed class PaintServerManager
{
    ...
    private readonly Dictionary<string, PaintServer> m_servers = [];
    ...
    public string Create(SVG svg, XmlNode node)
    {
        if (node.Name == SVGTags.sLinearGradient)
        {
            ...
            if (m_servers.ContainsKey(id) == false) m_servers[id] = new LinearGradientColorPaintServer(this, node);
            return id;
        }
        if (node.Name == SVGTags.sRadialGradient)
        {
            string id = XmlUtil.AttrValue(node, "id");
            if (m_servers.ContainsKey(id) == false) m_servers[id] = new RadialGradientColorPaintServer(this, node);
            return id;
        }
        if (node.Name == SVGTags.sPattern)
        {
            string id = XmlUtil.AttrValue(node, "id");
            if (m_servers.ContainsKey(id) == false) m_servers[id] = new PatternPaintServer(this, svg, node);
            return id;
        }
        return null!;
    }
    ...
    public PaintServer GetServer(string serverKey)
    {
        ...
        if (serverKey != null && m_servers.TryGetValue(serverKey, out PaintServer? result))
            return result;
        else
            return null!;
    }
    ...
    public string Parse(string value)
    {
        try
        {
            ...
            if (this.m_servers.TryGetValue(value, out PaintServer? result)) return value;
    ...
    private string ParseSolidColor(string value)
    {
        ...
        if (m_servers.TryGetValue(id, out _)) return id;
        PaintServer? result = new SolidColorPaintServer(this, ParseHexColor(value));
        m_servers[id] = result;
        return id;
    }
    ...
    {
        LoadKnownColors();
        if (m_servers.TryGetValue(value, out _)) return value;
        if (m_knownColors.TryGetValue(value, out Color c))
        {
            PaintServer? result = new SolidColorPaintServer(this, c);
    ...
    {
        m_knownColors ??= [with(StringComparer.OrdinalIgnoreCase)];
