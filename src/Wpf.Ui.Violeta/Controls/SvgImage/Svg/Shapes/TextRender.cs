...
public sealed partial class TextRender : TextRenderBase
{
    ...
    {
        using TextRenderState state = new();
        if (!state.Setup(text))
        {
    ...
    {
        public List<GlyphRun> GlyphRuns { get; set; } = [];
        public Dictionary<GlyphRun, List<Rect>> BackgroundDecorations { get; set; } = [];
        public Dictionary<GlyphRun, List<Rect>> ForegroundDecorations { get; set; } = [];
        public Dictionary<GlyphRun, Rect> GlyphRunBounds { get; set; } = [];
        public Dictionary<GlyphRun, TextStyle> TextStyles { get; set; } = [];
        public Dictionary<GlyphRun, TextShapeBase> TextContainers { get; set; } = [];
    ...
    private static GeometryGroup CreateGeometry(TextShape root, TextRenderState state)
    {
        ...
        TextStyleStack textStyleStacks = new();
        PopulateTextStrings(textStrings, root, textStyleStacks);
        GeometryGroup mainGeometryGroup = new();
        ...
        foreach (TextString textString in textStrings)
        {
            ...
            if (CreateRun(textString, state, font, isSideways, baselineOrigin, out Point newBaseline, out bool newTextChunk, ref currentTextAlignment) is GlyphRun run)
            {
                if (newTextChunk)
                {
    ...
    private static void GetTextDecorations(Geometry geometry, TextStyle textStyle, Typeface font, Point baselineOrigin, out List<Rect> backgroundDecorations, out List<Rect> foregroundDecorations)
    {
        ...
        foreach (TextDecorationLocation textDecorationLocation in textStyle.TextDecoration.Select(td => td.Location))
        {
            if (textDecorationLocation == TextDecorationLocation.Strikethrough)
            {
                ...
                Rect bounds = new(geometry.Bounds.Left, decorationPos, geometry.Bounds.Width, decorationThickness);
    ...
    private static GlyphRun CreateRun(TextString textString, TextRenderState state, Typeface font, bool isSideways, Point baselineOrigin, out Point newBaseline, out bool newTextChunk, ref TextAlignment currentTextAlignment)
    {
        ...
        var advanceWidths = WrapInThousandthOfEmRealDoubles(renderingEmSize, [.. glyphIndices.Select(c => glyphFace.AdvanceWidths[c] * renderingEmSize)]);
        ...
        GlyphRun run = new(glyphFace, state.BidiLevel, isSideways, renderingEmSize,
    ...
    private static readonly ConstructorInfo _thousandthOfEmRealDoublesConstructor = _thousandthOfEmRealDoublesType?.GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null, [typeof(double), typeof(IList<double>)], null)!;
    ...
    {
        return (IList<double>)_thousandthOfEmRealDoublesConstructor?.Invoke([renderingEmSize, advanceWidths])! ?? advanceWidths;
    }
