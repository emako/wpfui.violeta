...
public class LengthPercentageOrNumberList : IList<LengthPercentageOrNumber>
{
    ...
    private List<LengthPercentageOrNumber> _list = [];
    private readonly LengthOrientation _orientation;

    [SuppressMessage("Performance", "SYSLIB1045:Convert to 'GeneratedRegexAttribute'.")]
    private static readonly Regex _splitRegex = new(@"\b(?:,|\s*,?\s+)\b", RegexOptions.Compiled);
    ...
    private void Parse(string value)
    {
        ...
        _list = [.. list.Select(s => LengthPercentageOrNumber.Parse(_owner, s, _orientation))];
    }
