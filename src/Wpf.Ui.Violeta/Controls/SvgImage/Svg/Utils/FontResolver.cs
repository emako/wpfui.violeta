...
{
    private readonly ConcurrentDictionary<string, string> _fontCache = new();
    ...
    public FontResolver(int maxLevenshteinDistance = 0)
    {
        ...
        _normalizedFontNameMap = [with(_availableFonts.Count)];
    ...
    private static readonly Regex _normalizationRegex = new(@"[\s\-_]", RegexOptions.Compiled);
