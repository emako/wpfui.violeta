...
public class SVGRender
{
    ...
    internal DrawingGroup LoadGroup(IList<Shape> elements, Rect? viewBox, bool isSwitch)
    {
        ...
        foreach (Shape shape in elements)
        {
            ...
            if (shape is UseShape useshape)
            {
                ...
                if (currentUsedShape != null)
                {
                    ...
                    if (currentUsedShape is Group group)
                        subgroup = this.LoadGroup(group.Elements, null, false);
                    else
                        subgroup = this.LoadGroup([currentUsedShape], null, false);
                    ...
                    if (useshape.Transform != null)
                        ...
                        {
                            Children = [subgroup.Transform, useshape.Transform]
                        };
            ...
            {
                LineGeometry line = new(lineShape.P1, lineShape.P2);
            ...
            {
                EllipseGeometry c = new(new Point(circle.CX, circle.CY), circle.R, circle.R);
            ...
            {
                EllipseGeometry c = new(new Point(ellipse.CX, ellipse.CY), ellipse.RX, ellipse.RY);
                var di = this.NewDrawingItem(shape, c);
                AddDrawingToGroup(grp, shape, di);
                continue;
            }
            ...
            if (shape is PathShape pathShape)
            {
                ...
                if (pathShape.Fill == null || pathShape.Fill.IsEmpty(svg))
                {
                    if (pathShape.Stroke == null || pathShape.Stroke.IsEmpty(svg))
                    {
                        Fill fill = new(svg)
                        {
                            PaintServerKey = this.SVG.PaintServers.Parse("black")
                        };
                        pathShape.Fill = fill;
                    }
                }
    ...
    private static IEnumerable<Geometry> GetStyledSpans(Geometry geometry)
    {
        ...
        {
            if (TextRender.GetElement(gg) is not null)
            {
