...
/// <summary>
/// Static confetti / salute effect rendered on an AdornerLayer.
/// Call <see cref="Fire()"/> or <see cref="Fire(Options)"/> to launch particles.
/// </summary>
public class ConfettiCannon
{
    ...
    private static ConfettiCannon EnsureConfettiCannon(string? token)
    {
        ...
        ConfettiCannons[token!] = confettiCannon;
        return confettiCannon;
    }
    ...
    private static AdornerLayer? GetAdornerLayer(string? token)
    {
        ...
        if (string.IsNullOrEmpty(token))
        {
            ...
            decorator = window?.FindDescendant<AdornerDecorator>();
        }
        else
        {
            TokenManager.TryGetControl(token!, out FrameworkElement? element);
            decorator = element is Window
                ? element.FindDescendant<AdornerDecorator>()
                : element?.FindDescendant<ConfettiCannonContainer>();
        }
    ...
    private void AddConfettis(Options options)
    {
        ...
        for (int i = 0; i < options.ParticleCount; i++)
        {
            _confettis.Enqueue(new Confetti
            {
                ...
                Brush = brushes[i % brushes.Count].CloneCurrentValue(),
    ...
    {
        var adorner = _offscreenVisual.FindAscendant<VisualAdornerContainer>();
