...
public partial class TeachingTip : ContentControl, IControlProtected
{
    ...
    private static readonly ControlStrings ResourceAccessor = new(typeof(TeachingTip), ModernControlCategory.Windows);
    ...
    public override void OnApplyTemplate()
    {
        ...
        m_contentRootGrid = GetTemplateChildT<Border>(s_contentRootGridName, controlProtected);
        ...
        if (alternateCloseButton != null)
        {
            ...
            ToolTip tooltip = new()
            {
                Content = ResourceAccessor.GetLocalizedStringResource(SR_TeachingTipAlternateCloseButtonTooltip)
            };
    ...
    void IsOpenChangedToOpen()
    {
        ...
        if (window != null)
        {
            m_currentBoundsInCoreWindowSpace = TransformToVisual(window).TransformBounds(
                new Rect(
                    0.0,
                    0.0,
                    (float)ActualWidth,
        ...
        Rect GetCurrentTargetBoundsInCoreWindowSpace()
        {
            ...
            if (target != null)
            {
                ...
                {
                    return target.TransformToVisual(targetWindow).TransformBounds(
                        new Rect(
                            0.0,
                            0.0,
...
#pragma warning restore IDE0150 // Prefer 'null' check over type check
    ...
    private void RaiseClosingEvent(bool attachDeferralCompletedHandler)
    {
        ...
        {
            TeachingTipDeferral instance = new TeachingTipDeferral(() =>
            {
                if (!args.Cancel)
                {
    ...
    private void OnTargetChanged()
    {
        ...
        if (IsOpen)
        {
            if (target != null)
            {
                ...
                if (targetWindow != null)
                {
                    m_currentTargetBoundsInCoreWindowSpace = target.TransformToVisual(targetWindow).TransformBounds(
                        new Rect(
                            0.0,
                            0.0,
                            (float)target.ActualWidth,
    ...
    private void RepositionPopup()
    {
        if (IsOpen)
        {
            ...
            var newCurrentBounds = TransformToVisual(window).TransformBounds(
                new Rect(
                    0.0,
                    0.0,
                    (float)ActualWidth,
    ...
    private void UpdatePopupRequestedTheme()
    {
        ...
        if (frameworkElement6 != null)
        {
            ...
            {
                // Theme sync not required in Violeta (shared app resources).
